...
class DDizi : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        ...
        "${mainUrl}/yerli-diziler/page/"       to "Yerli Diziler",
        "${mainUrl}/yabanci-diziler/page/"     to "Yabancı Diziler",
        "${mainUrl}/eski-dizi-arsivi/page/"    to "Eski Diziler"
    )

    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val document = app.get("${request.data}${page}/").document
        val home     = document.select("div.post").mapNotNull { it.diziler() }

        return newHomePageResponse(request.name, home)
    }

    private fun Element.diziler(): SearchResponse? {
        val title     = this.selectFirst("h2.title")?.text()?.substringBefore(" izle") ?: return null
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val poster      = fixUrlNull(document.selectFirst("div.afis img")?.attr("src"))
        ...
        val episodes    = document.select("div.bolumler a").mapNotNull {
